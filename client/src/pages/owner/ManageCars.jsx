...
const ManageCars = () => {
  ...
  const fetchOwnerCars = async () => {
    try {
      ...
      if (error.response?.status === 401) toast.error('Please log in to view your cars.')
      else if (error.message.includes('Network Error')) toast.error('Network error. Please check your internet connection and refresh the page.')
      else toast.error('Failed to load your cars. Please refresh the page or contact support if the problem persists.')
    }
  }
  ...
  const toggleAvailability = async (carId) => {
    try {
      ...
      else if (error.response?.status === 401) toast.error('Please log in to update car availability.')
      else if (error.message.includes('Network Error')) toast.error('Network error. Please check your internet connection.')
      else toast.error('Failed to update car availability. Please try again or contact support if the problem persists.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const deleteCar = async (carId) => {
    try {
      ...
      if (error.response?.data?.message) toast.error(error.response.data.message)
      else if (error.response?.status === 401) toast.error('Please log in to delete cars.')
      else if (error.response?.status === 403) toast.error('You can only delete your own cars.')
      else if (error.message.includes('Network Error')) toast.error('Network error. Please check your internet connection.')
      else toast.error('Failed to delete car. Please try again or contact support if the problem persists.')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    // NOTE: pt-16 / sm:pt-20 added to avoid overlap if topbar is fixed.
    // md:ml-56 / lg:ml-64 added to shift content if you have a fixed sidebar on larger screens.
    <div className="w-full min-h-screen box-border pt-16 sm:pt-20 lg:pt-24 px-3 sm:px-4 md:px-6 lg:px-10 md:ml-56 lg:ml-64">
      <Title
        title="Manage Cars"
        subTitle="View all listed cars, update their details, or remove them from the booking platform."
      />

      <div className="max-w-6xl w-full rounded-lg overflow-hidden border border-borderColor mt-4 sm:mt-6 bg-white shadow-sm">
                  ...
                  <tr key={index} className="border-t border-borderColor hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-3 sm:p-4"> 
                      <div className="flex items-center gap-2 sm:gap-3">
                        <button onClick={() => handleEditCar(car)} className="p-2 sm:p-3 hover:bg-blue-50 rounded-lg transition-colors border border-blue-200 bg-blue-25" title="Edit car">
                          <img src={assets.edit_icon} alt="Edit" className="w-5 h-5 sm:w-6 sm:h-6" />
                        </button>
                        <button onClick={() => handleToggleAvailability(car)} className="p-2 sm:p-3 hover:bg-gray-100 rounded-lg transition-colors border border-gray-200 bg-gray-25" title={car.isAvaliable ? 'Hide car' : 'Show car'}>
                          <img src={car.isAvaliable ? assets.eye_close_icon : assets.eye_icon} alt="Toggle visibility" className="w-5 h-5 sm:w-6 sm:h-6" />
                        </button>
                        <button onClick={() => handleDeleteCar(car)} className="p-2 sm:p-3 hover:bg-red-50 rounded-lg transition-colors border border-red-200 bg-red-25" title="Delete car">
        ...
        {/* MOBILE: stacked card view (smaller screens) */}
        <div className="sm:hidden p-3 space-y-3">
          {cars.map((car, idx) => (
            <div key={idx} className="bg-white border border-borderColor rounded-lg p-3 shadow-sm">
              <div className="flex items-start gap-3">
                <img src={car.image} alt="" className="h-12 w-12 rounded-md object-cover flex-shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate">📍 {car.address?.city ? `${car.address.city}, ${car.address.state}` : car.location}</p>
                    </div>

                    <div className="flex flex-col items-end justify-start">
                      <p className="font-medium text-sm">{currency}{car.pricePerDay}/day</p>
                      <span className={`mt-2 px-2 py-1 rounded-full text-xs font-medium ${car.isAvaliable ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                        {car.isAvaliable ? "Available" : "Unavailable"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex gap-2">
                    <button onClick={() => handleEditCar(car)} className="flex-0 p-2 hover:bg-blue-50 rounded-lg transition-colors border border-blue-200 bg-blue-25" title="Edit car">
                      <img src={assets.edit_icon} alt="Edit" className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleToggleAvailability(car)} className="flex-0 p-2 hover:bg-gray-100 rounded-lg transition-colors border border-gray-200 bg-gray-25" title={car.isAvaliable ? 'Hide car' : 'Show car'}>
                      <img src={car.isAvaliable ? assets.eye_close_icon : assets.eye_icon} alt="Toggle" className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDeleteCar(car)} className="flex-0 p-2 hover:bg-red-50 rounded-lg transition-colors border border-red-200 bg-red-25" title="Delete car">
